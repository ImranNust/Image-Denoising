from utilities.model import BRDNet
...
def train(train_path, valid_path, model_path,
    ...
    if No_of_GPUs>1 and not BatchReNormalization:
        print('The number of GPUS are more than 1 and BatchNormalization is {}; Therefore, deploying distributed training across all {} GPUs'.format(BatchReNormalization, No_of_GPUs))
        strategy = tf.distribute.MirroredStrategy()
        # print("Number of devices: {}".format(strategy.num_replicas_in_sync))
        # Open a strategy scope.
        with strategy.scope():
            mymodel = BRDNet(input_shape = input_shape)
        ...
        print('The number of GPUS are less than 1 or BatchNormalization is {}; Therefore, deploying distributed training across 1 GPUs'.format(BatchReNormalization))
        mymodel = BRDNet(input_shape = input_shape, BatchReNormalization = BatchReNormalization)
        mymodel.compile(optimizer=tf.keras.optimizers.Adam(learning_rate = lr),
                            loss = tf.keras.losses.MeanSquaredError())
        history = mymodel.fit(x=input_train, y = label_train,
                              epochs=epochs, verbose=1, validation_data=(input_valid, label_valid),
